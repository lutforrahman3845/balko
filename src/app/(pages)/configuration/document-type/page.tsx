"use client"
import { DocumentTypeHeader } from "@/components/DocumentType/DocumentTypeHeader";
import DocumentTypeTable from "@/components/DocumentType/DocumentTypeTable";
...
const Page = () => {
    ...
    return (
        <>
            <DocumentTypeHeader data={filteredData} />
            {isError ? (
                <>
                ...
                <>
                                ...
                                <div className="flex items-center gap-3 sm:gap-4 pr-3 sm:pr-6 border-r border-zinc-200 dark:border-zinc-800">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[13px] sm:text-sm font-semibold whitespace-nowrap leading-none">
                                            {selectedIds.length}{" "}
                                            {selectedIds.length === 1 ? "Document Type" : "Document Types"}
                                        </span>
                    ...
                    {isDialogOpen && (
                        <ConfirmDialog
                            isOpen={isDialogOpen}
                            type="danger"
                            title="Delete Document Types"
                            onClose={() => setIsDialogOpen(false)}
                            onCancel={() => setIsDialogOpen(false)}
                            confirmButtonType="destructive"
                            onConfirm={() => {
                                toast.success(
                                    `${selectedIds.length} document types deleted successfully`,
                                );
                                setRowSelection({});
                        ...
                        >
                            <span>
                                ...
                                {selectedIds.length === 1 ? "document type" : "document types"}? This action
                                cannot be undone.
                            </span>
                        </ConfirmDialog>
                    )}

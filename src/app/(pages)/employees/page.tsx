...
const Page = () => {
    ...
    const { data: roles } = useGetRolesQuery({ pageSize: 100 });
    ...
    const { data: employees, isLoading, isError, refetch } = useGetEmployeesQuery({
        ...
        employeeType: selectedTypes.join(","),
    });
                ...
                <>
                                ...
                                <div className="flex items-center gap-3 sm:gap-4 pr-3 sm:pr-6 border-r border-zinc-200 dark:border-zinc-800">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[13px] sm:text-sm font-semibold whitespace-nowrap leading-none">
                                            {selectedIds.length} {selectedIds.length === 1 ? "Employee" : "Employees"}
                                        </span>
                    ...
                    {isDialogOpen && (
                        <ConfirmDialog
                            ...
                            onConfirm={() => {
                                toast.success(`${selectedIds.length} Employee deleted successfully`);
                                setRowSelection({});
                                setIsDialogOpen(false);
                            }}
                        >
                            <span>
                                Are you sure you want to delete {selectedIds.length} selected {selectedIds.length === 1 ? "Employee" : "Employees"}? This action cannot be undone.
                            </span>
                        </ConfirmDialog>
                    )}
                </>

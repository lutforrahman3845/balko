...
/**
 * Presentational kanban board: renders columns, handles drag-and-drop, and
...
const TaskBoardView = ({ tasks, isLoading, isError, onRetry }: TaskBoardViewProps) => {
  ...
  // Re-seed whenever a fresh task list arrives (render-phase sync, so the board
  // tracks the server without an effect / cascading render).
  const [seededFrom, setSeededFrom] = useState<ExpandedTask[] | undefined>(undefined);
  if (tasks !== seededFrom) {
    setSeededFrom(tasks);
  ...
  return (
    <>
      <div className="w-full overflow-x-auto custom-scrollbar p-4">
